...
def multi_trial(n_trials=90, stop_trial_prob = 1/3, gabor_freq_prob = 0.5, stim_dir = "stimuli"):
    ...
    stop_trials = stats.bernoulli.rvs(stop_trial_prob, 0, n_trials)
    ...
    gabor_freq_set = stats.bernoulli.rvs(gabor_freq_prob, 0, n_trials)
    
    # randomly draw the length of cue intervals and response intervals from a uniform distribution
    snr_vec = [1/3 for _ in range(n_trials)]
    ...
    parameters = pd.DataFrame({'trial': list(range(n_trials)), 'correct_answer': correct_answer, 'only_noise_frames': noise_frames_use, 'only_gabor_frames': gabor_frames_use, 'gabor_annulus_frames': annulus_frames_use, 'frame_length': frame_length_use})
    parameters.to_csv(stim_dir + "/parameters.csv", index = False)
    
    return frame_length_use
    
multi_trial(n_trials = 90, stop_trial_prob = 1/3, gabor_freq_prob = 0.5)